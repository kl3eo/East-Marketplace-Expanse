...
export default function NFTCard ({ nft, action, updateNFT, onCliCliCli }) {
  ...
  const { setModalNFT, setIsModalOpen, isDescOpen, isReqFormOpen, isAnswerShown, setIsAnswerShown, currSize, currLang } = useContext(NFTModalContext)
  ...
  museoReal = Museo === 'slammuseum' ? 'Art Museum, Saint Louis' : museoReal // open access for PD https://www.slam.org/terms-of-use/
  museoReal = Museo === 'philamuseum' ? 'Museum of Art, Philadelphia' : museoReal // open access for PD https://philamuseum.org/legal
  ...
  museoReal = Museo === 'saam' ? 'Smithsonian AAM' : museoReal // https://www.si.edu/termsofuse cc0 icon
  museoReal = Museo === 'lacma' ? 'County Museum of Art, LA' : museoReal // without restriction on PD https://www.lacma.org/about/contact-us/terms-use
  museoReal = Museo === 'smkmuseum' ? 'SMK, Copenhagen' : museoReal // CC-PDM https://www.smk.dk/en/article/about-smk-open/ or https://www.smk.dk/en/section/use-of-smk-material/
  museoReal = Museo === 'artic' ? 'Art Institute, Chicago' : museoReal // open access with CC0 tag https://www.artic.edu/collection-information/open-access/open-access-images
  museoReal = Museo === 'dia' ? 'Detroit Institute of Arts' : museoReal // free for PD https://dia.org/collection/rights-reproduction
  museoReal = Museo === 'siedu' ? 'Smithsonian NMAA' : museoReal // open access cc0 https://www.si.edu/openaccess
  museoReal = Museo === 'kmska' ? 'KMSKA, Antwerpen' : museoReal // CC-PDM  https://www.kmska.be/en/disclaimer
  ...
  museoReal = Museo === 'imamuseum' ? 'Newfields Archives, IN' : museoReal // open access https://discovernewfields.org/licensing-resources
  museoReal = Museo === 'getty' ? 'Getty Museum, LA' : museoReal // any purpose Getty Open Content CC0 https://www.getty.edu/projects/open-content-program/
  museoReal = Museo === 'staedel' ? 'Städel Museum, Frankfurt' : museoReal // CC-PDM https://sammlung.staedelmuseum.de/en/concept
  museoReal = Museo === 'ghentmuseum' ? 'Ghent Museum, Belgium' : museoReal // CC-PDM icon&link on download https://www.mskgent.be/en/collection/image-rights
  ...
  museoReal = Museo === 'gemaldesmb' ? 'Staatliche Museen zu Berlin' : museoReal // CC-PDM or CC BY-SA https://www.smb.museum/en/open-science/use/
  museoReal = Museo === 'artsmia' ? 'Minneapolis Institute of Art' : museoReal // CC-PDM https://new.artsmia.org/copyright-and-image-access
  ...
  museoReal = Museo === 'yaleartgallery' ? 'Yale Art Gallery, CT' : museoReal // open access https://artgallery.yale.edu/using-collection/using-images
  museoReal = Museo === 'lenbachhaus' ? 'Lenbachhaus, Munchen' : museoReal // tags with Creative Commons license CC0 1.0 are free to dnld
  museoReal = Museo === 'centraalmuseum' ? 'Centraal Museum, Utrecht' : museoReal // low-res dnld free on PD https://www.centraalmuseum.nl/en/general-terms-conditions
  museoReal = Museo === 'wienmuseum' ? 'Wien Museum, Vienna' : museoReal // CC0 or CC-BY https://sammlung.wienmuseum.at/en/about-us/
  museoReal = Museo === 'nasjonalmuseet' ? 'Nasjonalmuseet, Oslo' : museoReal // public domain free use CC-BY
  museoReal = Museo === 'albertina' ? 'Albertina Museum, Vienna' : museoReal // PD free dnld resolution to 1200px https://www.albertina.at/en/research/image-reproduction-services/
  ...
  museoReal = Museo === 'artsbma' ? 'Birmingham Museum of Art, AL' : museoReal // open access for PD https://www.artsbma.org/collection-resources/collections-access-policy/
  museoReal = Museo === 'dmamuseum' ? 'Dallas Museum of Art, TX' : museoReal // open access for PD (notice appears on download)
  museoReal = Museo === 'karlsruhe' ? 'Staatliche Kunsthalle, Karlsruhe' : museoReal // CC0 for PD https://www.kunsthalle-karlsruhe.de/en/cc0/
  ...
  museoReal = Museo === 'thorvald' ? 'Thorvaldsen\'s Museum, Denmark' : museoReal // CC0 https://kataloget.thorvaldsensmuseum.dk/en/colophon/copyright
  museoReal = Museo === 'muzee' ? 'Museum of Modern Art, Ostend' : museoReal // CC-PDM icon&link on dnld
  museoReal = Museo === 'jewishmuseum' ? 'The Jewish Museum, New York' : museoReal // PD notice appears on dnld button
  museoReal = Museo === 'freiburgneuekunst' ? 'Museum für Neue Kunst, Freiburg' : museoReal // CC BY 4.0
  museoReal = Museo === 'freiburgaugustiner' ? 'Augustiner Museum, Freiburg' : museoReal // CC BY 4.0
  museoReal = Museo === 'sngmuseum' ? 'Slovak NG, Bratislava' : museoReal // CC-PDM
  museoReal = Museo === 'stuttstaats' ? 'Staatsgalerie Stuttgart' : museoReal // CC-PDM and downlowd button appears on Zoom In
  museoReal = Museo === 'hlmdmuseum' ? 'Hessisches Landesmuseum, Darmstadt' : museoReal // CC BY-SA https://www.hlmd.de/en/hlmd/photo-requests
  museoReal = Museo === 'ateneum' ? 'Ateneum, Helsinki' : museoReal // Copyright-free icon and downlownd in good quality offered; PD assumed
...
*/
  ...
  // if (isReqFormOpen) clickerSigned = <div style={{ minWidth: '48px', minHeight: '16px', width: '8vw', height: '1vw', borderRadius: isMobile ? '24px' : '0.5vw', background: '#fff', position: 'absolute', top: '0vw', right: '10px', zIndex: '999', fontStyle: 'italic', lineHeight: isMobile ? '36px' : '0.6 vw', fontSize: isMobile ? '18px' : '12px', fontFamily: 'Times', textAlign: 'center' }}>{currLang === 'RU' ? 'ЗАГРУЗИТЬ ЕЩЁ' : 'CLICK TO SKIP'}</div>
  ...
  function handleCardImageClick () {
    ...
    if (isWebRTC || isExcel || isReqFormOpen || isMobile) return
  ...
  const stringLengthLimitUgade = 42
  const stringLengthLimit = !isReqFormOpen ? 24 : 0
  const stringLengthLimitDesc = !isReqFormOpen ? 24 : 0
  const clickToOpen = isReqFormOpen ? currLang === 'RU' ? 'ПОКАЗАТЬ ОТВЕТ' : 'SHOW ME ANSWER!' : '..'
  // const descToOpen = isReqFormOpen ? currLang === 'RU' ? 'ОТКРЫТЬ РАБОТЫ ХУДОЖНИКА' : 'MORE WORKS BY THIS ARTIST' : '..'
  const descToOpen = isReqFormOpen ? '' : '..'
  const shortName = currName.length > stringLengthLimit ? currName.substr(0, stringLengthLimit) + clickToOpen : currName
  const shortDesc = description.length > stringLengthLimitDesc ? description.substr(0, stringLengthLimitDesc) + descToOpen : description // max 3 rows of text
  const currNameNoYears = currName.split(/\./); const currNameNoYear = currNameNoYears[0]
  ...
  return (
    !isEdited
      ? <Card
        ...
        {Memoized}
        {museoReal !== '' && !isReqFormOpen && <div style={{ backgroundColor: '#fff', textAlign: 'right', lineHeight: '16px', fontSize: '16px' }}><NavMuseo onClick={emptyBucket} title={museoReal} href={museoHref} col={museoCol} /></div>}
        {museoReal === '' && typeof window !== 'undefined' && window.location.hostname === 'canneverbe' + '.' + currentDomain && <div style={{ fontSize: '8px' }}>&nbsp;</div>}
        <CardContent id={'cc_' + tId} style={{ backgroundColor: '#fff', minHeight: '50px', marginTop: isReqFormOpen ? isAnswerShown ? '-4px' : '0px' : '-4px', display: scalingAllowed && hiResScreen ? 'none' : 'block' }} className={classes.cardContent} onClick={(e) => { if (isReqFormOpen) e.stopPropagation(); setIsAnswerShown(!isAnswerShown); if (document.querySelector('.parentCont')) document.querySelector('.parentCont').blur() }}>
          {((isReqFormOpen && !isAnswerShown) || !isReqFormOpen) && <div className='parentCont' style={{ position: 'relative', textAlign: isReqFormOpen ? 'center' : 'left', cursor: 'pointer' }}><NFTName name={(isDescOpen && name.length < stringLengthLimit) || isAnswerShown ? isReqFormOpen ? currName.length <= stringLengthLimitUgade ? currName : currNameNoYear.substr(0, stringLengthLimitUgade) + '.. ' + currName.substr(currName.length - 4, 4) : currName : shortName} variant={isReqFormOpen ? 'h8' : notebook ? 'h6' : 'h6'} style={{ marginBottom: isReqFormOpen ? '-12px' : '0px' }}/></div>}
          {!badThingsHappen && typeof window !== 'undefined' && window.location.hostname !== 'mydocs' + '.' + currentDomain && window.location.hostname !== 'split' + '.' + currentDomain && ((isReqFormOpen && isAnswerShown) || !isReqFormOpen) && <div className='parentCont' onClick={ (e) => { if (isReqFormOpen) { e.stopPropagation(); if (document.querySelector('.parentCont')) document.querySelector('.parentCont').children[1].click() } } } style={{ border: '0px solid #900', position: 'relative', textAlign: isReqFormOpen ? 'center' : 'left', cursor: 'pointer' }}><div style={{ height: isReqFormOpen ? '3px' : '0px' }}></div><NavDesc onClick={emptyBucket} title={isShortDesc && !isAnswerShown ? shortDesc : description} href={href} /><div style={{ height: isReqFormOpen ? '6px' : '0px' }}></div></div>}
          {!badThingsHappen && typeof window !== 'undefined' && window.location.hostname === 'mydocss' + '.' + currentDomain && <div onClick={isShortDesc ? setIsShortDesc(false) : setIsShortDesc(true)}>{isShortDesc && !isAnswerShown ? shortDesc : description}</div>}
          ...
          <Box className={classes.addressesAndPrice} style={{ display: badThingsHappen ? 'none' : 'block' }}>
            ...
            <div onClick={e => e.stopPropagation()} className={classes.priceContainer} style={{ float: 'right', marginTop: mydocs ? '20px' : '0px', display: nft.isLocked || (mydocs && action !== 'transfer') ? 'none' : 'block' }}>
              {action === 'sell'
            ...
            </div><div style={{ float: 'left', marginLeft: '5px', fontSize: '16px', color: '#369', marginTop: mydocs ? '36px' : '0px', display: action === 'sell' || action === 'transfer' || action === 'burn' ? nft.isLocked ? 'none' : 'block' : 'none' }} onClick={onCliCliCli}>{action === 'sell' ? 'or Transfer' : action === 'transfer' ? 'or Burn' : 'or Sell'}</div>
            <div className={classes.priceContainer} style={{ float: 'right', display: nft.isLocked ? 'block' : 'none' }}>
              This Token is Locked. <span style={{ color: '#369', fontWeight: 'bold', cursor: 'pointer', textDecoration: 'underline' }} onClick={(e) => { e.stopPropagation(); document.getElementById('unlocker' + tId).style.display = 'table'; document.getElementById('cm_' + tId).style.display = 'none'; document.getElementById('cc_' + tId).style.display = 'none' }}>Unlock</span>&nbsp;<span style={{ color: '#59c', fontWeight: 'bold', cursor: 'pointer', textDecoration: 'underline' }} onClick={(e) => { e.stopPropagation(); setIsEdited(true) }}>Edit</span>
            </div>
          </Box>

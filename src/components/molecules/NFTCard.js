...
export default function NFTCard ({ nft, action, updateNFT }) {
  ...
  return (
    <Card
      ...
      >
      {isVideo && !loading ? <CardMedia className="MuiCardMedia-root MuiCardMedia-media" alt={name} image={image} component="video" controls onClick={handleCardVideoClick} sx={{ width: { videoW }, height: '195px' }} /> : isVideo && loading ? <CardMedia className={classes.media} alt={name} image={loadingUrl} component="a" onClick={handleCardImageClick} /> : isWebRTC && !loading ? <div style={{ backgroundColor: '#112', height: isMobile ? '270px' : '380px', width: '100%', position: 'relative', top: '0px' }}><iframe src={img} style={{ height: isMobile ? '270px' : '435px', width: '100%', position: 'relative', top: '0px' }}></iframe></div> : isWebRTC && loading ? <CardMedia className={classes.media} alt={name} image={image} component="a" onClick={handleCardImageClick} /> : <CardMedia className={classes.media} alt={name} image={image} component="a" onClick={handleCardImageClick} />}

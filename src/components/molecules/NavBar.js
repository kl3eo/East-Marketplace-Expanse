      ...
      <Container maxWidth="100%" sx={{ backgroundColor: '#001122' }}>
        <Toolbar disableGutters sx={{ backgroundColor: '#001122' }}>
          <Box sx={{ flexGrow: 1, display: 'flex', maxWidth: typeof window !== 'undefined' && window.location.hostname === 'ooc.' + currentDomain ? '38%' : '100%' }}>
            {isMobile ? <div style={{ width: '1px' }}></div> : (isReady || hasInit) && (window.innerWidth > 640 || !lightBgr) && pages.map(({ title, href }) => <div key={title} onClick={href === '/my-nfts' ? () => { console.log('do nothing') } : ifPath}><NavItem title={title} href={href} key={title} style={{ maxWidth: isMobile ? '24px' : '120px', fontSize: isMobile ? '12px' : '16px' }}/></div>)}
          ...
          {typeof window !== 'undefined' && (window.location.hostname === 'nft.' + currentDomain || window.location.hostname === 'supernft.' + currentDomain || motivation) && !isMobile && window.innerWidth > 640 && <Box sx={{ flexGrow: 1, display: 'flex' }}><div id='stats_bar' style={{ display: isMobile ? ' none' : 'block', color: '#fed', backgroundColor: 'transparent', fontSize: '18px', width: '390px', height: '42px', borderRadius: '0px', textAlign: 'center', padding: '2px', margin: '12px 0px 8px 0px', lineHeight: '30px' }}><div style={{ width: '380px', margin: '0 auto' }}><div id='globe' style={{ float: 'left', display: 'none' }}>{globeO}</div><div id='stats_bar_string' style={{ float: 'left', fontWeight: 'bold' }}></div><div style={{ clear: 'left' }}></div></div></div></Box>}
          ...
          {typeof window !== 'undefined' && (window.location.hostname === 'nft.' + currentDomain || window.location.hostname === 'supernft.' + currentDomain || motivation) && currLang === 'RU' && <BasicSelectFineArtsRus disabledBox={disableBox} placeHolder={disableBox} style={{ zIndex: '1004' }}/>}
          {typeof window !== 'undefined' && (window.location.hostname === 'nft.' + currentDomain || window.location.hostname === 'supernft.' + currentDomain || motivation) && currLang === 'EN' && <BasicSelectFineArts disabledBox={disableBox} placeHolder={disableBox} style={{ zIndex: '1004' }}/>}

...
export default function NFTCardList ({ nfts, setNfts, withCreateNFT }) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => { if (document.getElementById('getRoom') && window.location.hostname === 'ooc' + '.' + currentDomain) document.getElementById('getRoom').style.display = 'none'; if (document.getElementById('stats_bar')) document.getElementById('stats_bar').style.display = 'none' }, hideOffReq)
  ...
  function NFT ({ nft, index }) {
    ...
    if (nft.owner === account && ((nft.creator === account && nft.isLocked) || toBurn)) {
      return <NFTCard nft={nft} action="burn" updateNFT={async () => { const fData = new FormData(); fData.append('pass', 'lollol'); if (mydocs) fData.append('network', 'hd'); if (split96) fData.append('network', 'hd96'); await fetch('https://' + currentServer + '.' + currentDomain + currentServerPort + '/cgi/update_10_last.pl', { body: fData, method: 'post', enctype: 'multipart/form-data' }).then((response) => response.json()).then((result) => { console.log('update_10_last1 result', result); updateAfterBurn(index, nft.tokenId) }).catch(async (err) => { console.log('Fetch1 update_10_last Error', err); await fetch('https://' + resServer + '.' + currentDomain + resServerPort + '/cgi/update_10_last.pl', { body: fData, method: 'post', enctype: 'multipart/form-data' }).then((response) => response.json()).then((result) => { console.log('update_10_last2 result', result); updateAfterBurn(index, nft.tokenId) }).catch((err) => { console.log('Fetch2 update_10_last Error', err) }) }); const emailHtml = render(<EmailCard nft={nft} mes={values.message.length ? values.message : 'by ' + nft.owner} who={values.subject.length ? values.subject : 'selling token ' + nft.tokenId} />); const req = await sendEmail(values.email.length ? values.email : 'alexshevlakov@yandex.ru', description, emailHtml); console.log('send_email req is', req) }} onCliCliCli={onCliCliCli} />
    }

    if (nft.owner === account && nft.creator === account && !nft.isLocked && !transferShown) {
      return <NFTCard nft={nft} action="sell" updateNFT={async () => { const fData = new FormData(); fData.append('pass', 'lollol'); if (mydocs) fData.append('network', 'hd'); if (split96) fData.append('network', 'hd96'); await fetch('https://' + currentServer + '.' + currentDomain + currentServerPort + '/cgi/update_10_last.pl', { body: fData, method: 'post', enctype: 'multipart/form-data' }).then((response) => response.json()).then((result) => { console.log('update_10_last1 result', result); updateAfterBurn(index, nft.tokenId) }).catch(async (err) => { console.log('Fetch1 update_10_last Error', err); await fetch('https://' + resServer + '.' + currentDomain + resServerPort + '/cgi/update_10_last.pl', { body: fData, method: 'post', enctype: 'multipart/form-data' }).then((response) => response.json()).then((result) => { console.log('update_10_last2 result', result); updateAfterBurn(index, nft.tokenId) }).catch((err) => { console.log('Fetch2 update_10_last Error', err) }) }); const emailHtml = render(<EmailCard nft={nft} mes={values.message.length ? values.message : 'by ' + nft.owner} who={values.subject.length ? values.subject : 'selling token ' + nft.tokenId} />); const req = await sendEmail(values.email.length ? values.email : 'alexshevlakov@yandex.ru', description, emailHtml); console.log('send_email req is', req) }} onCliCliCli={onCliCliCli} />
    }
    ...
    if (nft.owner === ethers.constants.AddressZero && nft.seller === account && !nft.sold && !nft.isLocked) {
      return <NFTCard nft={nft} action="cancel" updateNFT={async () => { const fData = new FormData(); fData.append('tok', nft.tokenId); fData.append('pass', 'lollol'); if (mydocs) fData.append('network', 'hd'); if (split96) fData.append('network', 'hd96'); fData.append('loco', location.hostname); await fetch('https://' + currentServer + '.' + currentDomain + currentServerPort + '/cgi/update_db_one.pl', { body: fData, method: 'post', enctype: 'multipart/form-data' }).then((response) => response.json()).then((result) => { console.log('update_db_one result', result); updateNFT(index, nft.tokenId) }).catch(async (err) => { console.log('Fetch update_db_one Error', err); await fetch('https://' + currentServer + '.' + currentDomain + currentServerPort + '/cgi/update_db_one.pl', { body: fData, method: 'post', enctype: 'multipart/form-data' }).then((response) => response.json()).then((result) => { console.log('update_db_one result', result); updateNFT(index, nft.tokenId) }).catch((err) => { console.log('Fetch update_db_one Error', err) }) }) }} onCliCliCli={onCliCliCli} />
    }

    if (nft.owner === ethers.constants.AddressZero && !nft.isLocked) {
      return <NFTCard nft={nft} action="buy" updateNFT={async () => { const emailHtml = render(<EmailCard nft={nft} mes={values.message.length ? values.message : 'by ' + nft.owner} who={values.subject.length ? values.subject : 'bought token ' + nft.tokenId} />); const req = await sendEmail(values.email.length ? values.email : 'alexshevlakov@yandex.ru', description, emailHtml); console.log('send_email req is', req); updateNFT(index, nft.tokenId); const fData = new FormData(); fData.append('tok', nft.tokenId); fData.append('pass', 'lollol'); if (mydocs) fData.append('network', 'hd'); if (split96) fData.append('network', 'hd96'); fData.append('loco', location.hostname); await fetch('https://' + currentServer + '.' + currentDomain + currentServerPort + '/cgi/update_db_one.pl', { body: fData, method: 'post', enctype: 'multipart/form-data' }).then((response) => response.json()).then((result) => { console.log('update_db_one result', result); updateNFT(index, nft.tokenId) }).catch(async (err) => { console.log('Fetch update_db_one Error', err); await fetch('https://' + currentServer + '.' + currentDomain + currentServerPort + '/cgi/update_db_one.pl', { body: fData, method: 'post', enctype: 'multipart/form-data' }).then((response) => response.json()).then((result) => { console.log('update_db_one result', result); updateNFT(index, nft.tokenId) }).catch((err) => { console.log('Fetch update_db_one Error', err) }) }) }} onCliCliCli={onCliCliCli} />
    }
  ...
  return (
    ...
    >
      ...
      <div id='contactShownBox' style={{ width: '310px', height: '300px', position: 'fixed', marginLeft: '32px', marginTop: '14px', fontSize: '20px', zIndex: '1011', textAlign: 'center', backgroundColor: '#9cf', display: 'none' }}>
        {typeof window !== 'undefined' && window.location.hostname === 'undesirable_behaviour' + '.' + currentDomain
          ? <form>

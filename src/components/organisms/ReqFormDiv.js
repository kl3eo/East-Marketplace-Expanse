...
import ReqTextCreation from '../molecules/ReqTextCreation'
...
export default function ReqFormDiv () {
  ...
  return (
        <div style={{ position: 'fixed', top: '0px', paddingTop: '0px', textAlign: 'center', width: '100%', height: '100%', display: isReqFormOpen ? 'block' : 'none', backgroundColor: '#001122', zIndex: '100001' }} >
          {currLang === 'EN' && !isMobile && <div style={{ position: 'relative', height: '15%', width: '100vw', backgroundColor: '#99ccff', fontSize: isMobile ? '16px' : '36px', lineHeight: '42px', paddingTop: isMobile ? '2px' : '16px' }}>Create and Own a <span onClick={ (e) => { e.stopPropagation(); setTextOnly(!textOnly) } } style={{ cursor: textOnly ? 'pointer' : 'default', color: textOnly ? '#009' : '#000', textDecoration: !textOnly ? 'none' : 'underline' }}>Fine Arts</span>/<span onClick={ (e) => { e.stopPropagation(); setTextOnly(!textOnly) } } style={{ cursor: !textOnly ? 'pointer' : 'default', color: textOnly ? '#000' : '#009', textDecoration: textOnly ? 'none' : 'underline' }}>Poetry</span> NFT!<br/>Absolutely FREE - Room-House gives the gift!</div>}
          {currLang === 'RU' && !isMobile && <div style={{ position: 'relative', height: '15%', width: '100vw', backgroundColor: '#99ccff', fontSize: isMobile ? '16px' : '36px', lineHeight: '42px', paddingTop: isMobile ? '2px' : '16px' }}>Создать свой <span onClick={ (e) => { e.stopPropagation(); setTextOnly(!textOnly) } } style={{ cursor: textOnly ? 'pointer' : 'default', color: textOnly ? '#009' : '#000', textDecoration: !textOnly ? 'none' : 'underline' }}>Живопись</span>/<span onClick={ (e) => { e.stopPropagation(); setTextOnly(!textOnly) } } style={{ cursor: !textOnly ? 'pointer' : 'default', color: textOnly ? '#000' : '#009', textDecoration: textOnly ? 'none' : 'underline' }}>Поэзия</span>-NFT!<br/>Бесплатно - от Румхауса в подарок!</div>}
          <div style={{ display: 'flex', position: 'relative', height: '70%', minHeight: '520px', width: '100vw', backgroundColor: '#ccff99' }}>
            ...
            <div style={{ position: 'relative', height: '100%', width: '21vw', minWidth: '384px', minHeight: '640px', backgroundColor: '#990066', float: 'left' }}>
              <div id='reqformdiv2' onClick={ (e) => { e.stopPropagation() } } style={{ position: 'relative', top: '1vh', left: '-0.36vw', margin: '0 auto', width: '360px', maxHeight: '600px', height: '96%', zIndex: '10002' }} >
                {!textOnly && <ReqCardCreation updateParent={updateParentState}/>}
                {textOnly && <ReqTextCreation updateParent={updateParentState}/>}
              </div>
              ...
              {currLang === 'EN' && <div id='reqformdiv5' onClick={ (e) => { e.stopPropagation(); const yon = window.confirm('Saved key, really?'); if (yon) { if (typeof window !== 'undefined' && window.location.hostname === 'happyminter.' + currentDomain) { location.reload() } else { setIsReqFormOpen(false); if (document.getElementById('getRoom')) document.getElementById('getRoom').style.display = 'block'; document.getElementById('reqformdiv2').style.display = 'block'; document.getElementById('reqformdiv4').style.display = 'none'; document.getElementById('reqformdiv5').style.display = 'none' } } } } style={{ display: 'none', margin: '10px auto', position: 'relative', width: isMobile ? '360px' : '20vw', minWidth: '360px', color: '#fff', fontSize: isMobile ? '12px' : '16px', padding: '2px', backgroundColor: '#334455', cursor: 'pointer' }}>CLICK TO RETURN</div>}
              {currLang === 'RU' && <div id='reqformdiv5' onClick={ (e) => { e.stopPropagation(); const yon = window.confirm('Сохранили ключ, точно?'); if (yon) { if (typeof window !== 'undefined' && window.location.hostname === 'happyminter.' + currentDomain) { location.reload() } else { setIsReqFormOpen(false); if (document.getElementById('getRoom')) document.getElementById('getRoom').style.display = 'block'; document.getElementById('reqformdiv2').style.display = 'block'; document.getElementById('reqformdiv4').style.display = 'none'; document.getElementById('reqformdiv5').style.display = 'none' } } } } style={{ display: 'none', margin: '10px auto', position: 'relative', width: isMobile ? '360px' : '20vw', minWidth: '360px', color: '#fff', fontSize: isMobile ? '12px' : '16px', padding: '2px', backgroundColor: '#334455', cursor: 'pointer' }}>Кликните, чтобы вернуться</div>}
            </div>
